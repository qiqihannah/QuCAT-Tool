...
def comb_testing_scenario1(config_dic):
    ...
    with open(test_result_file, 'w') as result_file:
        ...
        for i in range(len(list_data)):
            ...
            result_file.write('\n')
    print("number of failing test cases:" + str(count_fail))
...
def comb_testing_scenario2(config_dic):
    result_root = os.path.join(config_dic['program_folder'], config_dic['module_name'], config_dic['module_name']+'_ScenarioTwo')
    if not os.path.exists(result_root):
        os.makedirs(result_root)
    ...
    print("The number of test suite size is " + str(count_fail) + '.')
    print("The execution time for running the quantum program is " + str(execution_time) + "s.")
